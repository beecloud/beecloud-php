...
try {
    ...
    foreach($bills as $list) {
        $strParams = "?refund_no=".$refund_no."&bill_no=".$list->bill_no."&refund_fee=".$list->total_fee;
        $refund_url = in_array($list->sub_channel, array('WX_NATIVE', 'WX_SCAN', 'ALI_OFFLINE_QRCODE', 'ALI_SCAN')) ? 'ali.offline.qrcode/ali.qrcode.refund.php' : 'ali.agree.refund.php';
		$agree_refund = $list->spay_result ? "<a href='".$refund_url.$strParams."'>同意退款</a>" : "";
